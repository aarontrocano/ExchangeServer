...
foreach ($alias in $Import) {
    ...
    if ( ($objMailbox.ForwardingSMTPAddress -or $objMailbox.ForwardingAddress) ) {
        if ( ( $objMailbox.ForwardingSMTPAddress -match '*@amtrustgroup.com' ) ) {
            $Report += $objMailbox | Select-Object DisplayName,DeliverToMailboxandForward,ForwardingAddress,ForwardingSMTPAddress,PrimarySMTPAddress
        }
    }
...
$out = $Report
$out | Export-Csv -Path ([Environment]::GetFolderPath("Desktop")+'\MailboxFwdAmtReport.csv') -NoTypeInformation
$errorlog | Out-String | Set-Content ([Environment]::GetFolderPath("Desktop")+'\MailboxFwdAmtReport.txt')