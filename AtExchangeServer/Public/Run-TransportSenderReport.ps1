...
foreach ($mailbox in $Import) {
    $scriptBlock = {
        ...
        Get-TransportService | Get-MessageTrackingLog -ResultSize Unlimited -Start $dtStart -End $dtEnd -Sender $strAlias | Select-Object Timestamp,Serverhostname,Clienthostname,Source,EventId,Sender,Recipients,MessageSubject
    }
    $filename = ([Environment]::GetFolderPath("Desktop")+'\MailboxSenderReport_' + $mailbox.Alias + '.csv')
